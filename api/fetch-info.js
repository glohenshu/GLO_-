...
// 内容紹介の末尾に入るGLO連載の宣伝
//
// 出版実績ページの内容紹介は <span itemprop="description"> の中にあり、
// その末尾に次の2段落が足されるようになった。
//
//   <p>&nbsp;</p>
//   <p>幻冬舎ルネッサンス運営のWebメディア「ゴールドライフオンライン」にて連載中。</p>
//   <p><a href="https://renaissance-media.jp/…">▶試し読みはこちら</a></p>
//
// 内容紹介そのものの中にあるため、ページの構造では本文と区別できない。
// 文面が1種類に決まっているので文で切る。
//
// 2026/08/18時点では1件も無く、それ以降に順次追加されている
// （2026/09/04時点で gr1868〜1997 のうち56件）。
//
// STOP_MARKERS には入れない。あちらは著者紹介の打ち切りにも使うため、
// 内容紹介だけに効かせたい。
// ============================================================

// リンクの文言は「▶試し読みはこちら」と「試し読みはこちら」の2形がある。
// ▶を当てにせず、控えとして▶抜きで持つ。
// 保存済み416ページのうち、この語が宣伝ブロック以外に出るページは無い。
const INTRO_PROMO_MARKERS = [
  '幻冬舎ルネッサンス運営のWebメディア',
  '試し読みはこちら',
];


function cutIntroPromo(text) {
  let cut = text.length;

  for (const m of INTRO_PROMO_MARKERS) {
